...
public class WordleUtils {
	...
	public static String replace(String s, int index, char c) {
		...
		if (index>len) throw new IllegalArgumentException("Index outside of String");
		
		//return the string
		return s = s.substring(0, index)+c+ s.substring(index + 1);
	}
	...
	public static ArrayList<Boolean> isGreen(String guess, String answer) {
		...
		if(len!=answer.length()) {
			throw new IllegalArgumentException("Guess and Answer are different length");
		}
		
		ArrayList<Boolean> greenList= new ArrayList<>();
		
		//if string elements equal set the ArrayList to true
		for (int i=0; i<len; i++) {
			if (guess.charAt(i)==answer.charAt(i)) {
				greenList.add(i,true);
			}
			else {
				greenList.add(i,false);
			}
		}
		return greenList;
	...
	public static ArrayList<Boolean> isYellow(String guess, String answer) {
		...
		ArrayList<Boolean> greenList=WordleUtils.isGreen(guess,answer);
		
		ArrayList<Boolean> yellowList= new ArrayList<>();

		//iterate through green list and remove chars if green
		for(int k=0; k<len; k++) {
			if (greenList.get(k)==true) {
				answer=WordleUtils.replace(answer, k, '_');
				guess=WordleUtils.replace(guess, k, '*');
				len=answer.length();
			}
		}
		
		//iterate through guess and compare to answer
		for (int i=0; i<len; i++) {
			//default boolean is false (not yellow)
			boolean set=false;
			
			for(int j=0; j<len; j++) {
				if (guess.charAt(i)==answer.charAt(j)) {
					//if there is a match, set true and remove chars so no duplicates
					set=true;
					answer=WordleUtils.replace(answer, j, '_');
					guess=WordleUtils.replace(guess, j, '*');
					break;//break inside for loop because its already yellow
				}
			}
			yellowList.add(i,set);
		}
	...
	public static ArrayList<WordleColor> getColors(String guess, String answer) {
		...
		//get lists
		ArrayList<WordleColor> colorList= new ArrayList<>();
		ArrayList<Boolean> greenList=WordleUtils.isGreen(guess,answer);
		ArrayList<Boolean> yellowList=WordleUtils.isYellow(guess,answer);
		
		for(int i=0; i<len; i++) {
			if(greenList.get(i)==true) {
				//if 
				colorList.add(WordleColor.GREEN);
			}
			else if(yellowList.get(i)==true) {
				colorList.add(WordleColor.YELLOW);
			}
			else {
				colorList.add(WordleColor.GRAY);
			}
		}
	...
	public static void updateLetters(String guess, String answer, 
		...
		for (int i=0; i<len; i++) {
			if (colors.get(i)==WordleColor.GREEN || colors.get(i)==WordleColor.YELLOW) {
				//if green or yellow, then add to included
				possible.remove(guess.charAt(i));
				included.add(guess.charAt(i));
			}
			else if(included.contains(guess.charAt(i))){
				//if its already in included, skip
				continue;
			}
			else {
				//if its grey then add to excluded
				possible.remove(guess.charAt(i));
				excluded.add(guess.charAt(i));
			}
		}
		
		//if we know included letters, move all to excluded
		if(included.size()==guess.length()) {
